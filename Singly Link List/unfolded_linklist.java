   ...
   public static node fold(node head){
    ...
    public static node unfold(node head){
        ...
        node firsthead=head;
        node firstprevious=firsthead;
        node secondhead=head.next;
        node secondprevious=secondhead;
       
        while(secondprevious!=null&&secondprevious.next!=null){
            
            node f= secondprevious.next; //backup node
         
            firstprevious.next=secondprevious.next;
            secondprevious.next=f.next;
             firstprevious=firstprevious.next;
             secondprevious=secondprevious.next;
            
        }
        firstprevious.next=null;
        node sh=reverse(secondhead);
        firstprevious.next=sh;
        return firsthead;
    }
        ...
        LinkedList llist = new LinkedList();
           ...
           display(llist.head); 
              node liste4= fold(llist.head);
               System.out.println("Folded Linked list1:");
           display(llist.head); 
              node liste5= unfold(llist.head);
             System.out.println("unfolded Linked list1:");
             display(liste5);
    }
}
